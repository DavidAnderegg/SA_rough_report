...
When looking closer at a turbulent boundary layer, one can observe two
...
vales. It is possible to define a \textit{equivalent sand roughness}
$k_{seq}$ which has the same effect on the boundary layer as the
\textit{standard roughness} assumed so far. The author has found some formula
...
\subsubsection{The effect of roughness}
...
layer. This leads to an increase of the skin friction and ultimately drag.
Additionally, the separation behavior might be changed.
...
When looking at equation \ref{eq:overlap_law}, roughness simply changes the variable
$C^{+}$. Thus it may be summarized as follows:

\begin{table}[H]
  \centering
  \begin{tabular}{l l l}
    hydraulically smooth:   & $0 \leq k_{s}^{+} \leq 5$   & $C^{+} \approx 5.0$\\
    transition region\tablefootnote{Please note $C^{+}$ is
...
($\rho$) of a moving fluid are related. They are partial differential equations
which almost allway can't be solved analytically. Thus numerical methods are
needed.  The physical quantities depend on four variables: \textit{spatial
coordinates} (x, y, z) and the \textit{time} (t). As numerical methods are used,
those quantities need to be discretized \cite{nasaNS}.
...
not viable. To deal with it, simplifications are needed: (1) solving for a
...
\noindent Where $\rho u_{i}^{\prime} u_{j}^{\prime}$ are the \textit{six}
independent \textit{Reynolds-stresses}. Please note the fluctuating part for
pressure $p^{\prime}$ cancels out and does not appear again. The Reynolds-stresses
may be denoted in tensor notation:
...
\begin{itemize}
  ...
  \item Models where the eddy viscosity is not zero at wall. This can be
        ...
        According to \cite{sa_rough}, this approach works better for small
        roughness values.
\end{itemize}
...
slightly more complicated and also needs the friction velocity. This work
implemented the extension proposed by Boeing.
